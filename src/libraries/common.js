...
export const errorDebug = (error) => {
	...
	if (handleSpecialCasesOfErrors(error)) return;

	if (isNullOrEmpty(document.cookie) || localStorage.length == 0) {
	...
	if (error !== false && error.toString() !== 'Cancel') {
		...
		if (data) {
			ticker.addErrorMessage(data.message || Vue.prototype.getTranslation('I00.00024410', 'Error'));
...
/**
 * Handles special cases of errors. Returns true if error is handled.
 * @param {error object} error 
 * @returns true if this is a special case of error (and it is handled), false otherwise
 */
export const handleSpecialCasesOfErrors = (error) => {
	if ((((error || {}).response || {}).data || {}).debbugMessage && (((error || {}).response || {}).data || {}).debbugMessage.indexOf('System that you are trying to access does not exist') !== -1) {
		window.thisVue.$router.push('/errorPage');
		return true;
	}
	// add special cases of error handling here...
	return false;
}
...
var addAlertMessage = type => message => {
	Vue.prototype.storeState.alerts = Vue.prototype.storeState.alerts.filter(f => !(f.type === type && f.text === message));
	var alert = { type: type, text: message };
	Vue.prototype.storeState.alerts.push(alert);
	return () => Vue.prototype.storeState.alerts.remove(alert);
}
const alertFncs = {
	addErrorMessage: addAlertMessage('danger'),
	addWarningMessage: addAlertMessage('warning'),
	addInfoMessage: addAlertMessage('info'),
	addSuccessMessage: addAlertMessage('success'),
	removeMessage: message => Vue.prototype.storeState.alerts = Vue.prototype.storeState.alerts.filter(f => f.text !== message),
	clearMessages: () => {
		Vue.prototype.storeState.alerts = [];
	}
}

export const alert = alertFncs;
...
export const formatString = (item) => {
...
export const idExtendoString = (id) => {
	var keys = Object.keys(id);
	var idKey = keys.find(f => f.toLowerCase().endsWith('id'));
	var idMdbidKey = keys.find(f => f.toLowerCase().endsWith('id_mdbid'));
	var clvidKey = keys.find(f => f.toLowerCase().endsWith('clvid'));
	var itemid = keys.find(f => f.toLowerCase().endsWith('itemid'));
	return [id[idKey], id[idMdbidKey], id[clvidKey], id[itemid]].filter(f => !isNullOrEmpty(f)).join(':');
}
...
export const perPageItems = () => [...[5, 10, 15, 20, 25, 50, 100].map(f => ({ value: f, name: f })), { value: 1000000, name: Vue.prototype.getTranslation('I00.00012130', 'All') }];
