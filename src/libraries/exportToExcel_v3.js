    ...
    for (var i = 0; i < array.length; i++) {
        ...
        for (var j = 0; j < array_of_elements.length; j++) {
            if (!Vue.prototype.appSettings.exportToRealExcel) {
                ...
                if (!array_of_elements[j].imageName)
                    array_of_elements[j].imageName = array_of_elements[j].properName;
...
export const imagesToExcelString = (imgs) => imgs.length > 0 ? (Vue.prototype.appSettings.exportToRealExcel ? 'images->' + JSON.stringify(imgs) : "") : '';