<template>
	...
	<div class="row" @mousemove="itemDragging.moved" @mouseup="itemDragging.dropped" v-else>
		<nav class="actionMenu navbar HideOnPrint">
			<div class="d-flex flex-row flex-wrap ml-auto">
				<div class="actionLnk mr-4" @click="showPreview = true">
					<span class="icons icon-show plainIcn"></span>
					<u>{{ getTranslation('I00.00016540', 'Preview') }}</u>
				</div>
				<div class="actionLnk mr-4" @click="newApplication">
					<span class="icons icon-new plainIcn"></span>
					<u>{{ getTranslation('I00.00014710', 'New') }}</u>
				</div>
				<div class="actionLnk mr-4" :class="{ disabled: !isChanged }" @click="startSaving(false)">
					<span class="icons icon-save plainIcn"></span>
					<u>{{ getTranslation('I00.00004810', 'Save') }}</u>
				</div>
				<div class="actionLnk mr-4" :class="{ disabled: !isChanged && application.isPublished }" @click="publish">
					<span class="icons icon-publish plainIcn"></span>
					<u>{{ getTranslation('I00.00016550', 'Publish') }}</u>
				</div>
			</div>
		</nav>
		...
		<template v-else>
			...
			<div class="col-4" v-if="selected == null">
				<Sticky class="card" :marginTop="10" isInfobox="true">
					<ul class="nav nav-tabs smallTabs">
						<li class="nav-item" v-for="(tab, index) in tabs[activeFieldTab]" :key="index">
							<A class="nav-link tab" :class="{ active: fieldsTab === index }" @click="fieldsTab = index">{{ tab.name }}</A>
						</li>
					</ul>
					...
					<div class="tab-content tabWrapper withSticky overflow">
						<div class="tab-pane active">
							<div class="p-3" v-if="!tabHasVisible(tabs[activeFieldTab][fieldsTab])">
								<h6 class="mb-0">{{ getTranslation('I00.00049500', 'All fields are already in use!') }}</h6>
							</div>
							<div class="row no-gutters" v-if="filterVisible(tabs[activeFieldTab][fieldsTab].items).length > 0">
								<div class="col-4 py-3 position-relative borderRB text-center" v-for="(element, index) in filterVisible(tabs[activeFieldTab][fieldsTab].items)" :key="index">
									<button type="button" class="btn cornerBtn" v-tooltip="getTranslation('I00.00013100', 'Add')" @mousedown="addElement(element)"><span class="icons icon-plus plainIcn"></span></button>
									<p class="icons bigIcn plainIcn mt-2 mb-0 cursorDrag" :class="getIcon(element.icon || element.template)" :title="element.name" @mousedown="itemDragging.grabbed(null, element, $event)"></p>
			...
			<div class="borderDashTRBL" :style="movingStyle" v-if="movingElement != null">
				<div class="position-relative borderRB text-center">
					<p class="my-0 cursorDrag" :class="movingElement.icon != null ? 'icons bigIcn plainIcn whiteBg ' + getIcon(movingElement.icon || movingElement.template) : 'movingTab'" :title="movingElement.name">{{ movingElement.icon != null ? '' : movingElement.props.text }}</p>
				</div>
			</div>
...
import Section from '../../components/applicationForm/Section';
import Container from '../../components/applicationForm/Container';
import Properties from '../../components/applicationForm/Properties';
import Sticky from '../../components/common/sticky/Sticky';
import { deepClone, isNullOrEmpty, isBetween, getAllChildrens, ticker, errorDebug, hashCode } from '../../libraries/common';
import { createOtherFunctions, createValidationFunctions, defaultData, requestFields, reviewFields } from '../../libraries/applicationFormData';
import { validators } from '../../libraries/forms';
import eqnode from 'eqnode';
import { getAllChildren, logOut } from '../../libraries/vue';
...
export default {
  ...
  computed: {
    applicationTypes() {
      return [this.getTranslation('I00.00009540', 'Application'), this.getTranslation('I00.00009630', 'Review')]
    },
    options() {
