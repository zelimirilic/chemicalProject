<template>
	...
	<div :class="{ readOnly: requestId }" v-else>
		<Alerts additionalClass="sticky-top wp-640" v-if="!modalInModal" />
		...
		<div class="actionCard wp-640">
			...
			<template v-else>
				<Tickers class="mb-4" />
				<TabsContainer :sendBtn="true" @send="send" :childs="template.children" v-model="activeTab" @modalInModal="modalInModal = $event" :dat="data" additionalClass="pt-3 mb-0" />
			</template>
		</div>
	</div>
</template>
...
export default {
	...
	mounted() {
		...
		if (this.requestId) {
			axios
				.get(`/ApplicationRequest/Get?id=${this.requestId}`)
				.then((response) => {
