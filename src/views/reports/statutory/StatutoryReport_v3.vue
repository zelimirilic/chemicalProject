	...
	<Reports @tabChanged="tabChanged" :prodData="prodData">
		<template #filterSlot>
			<div class="filterSpecial form-inline borderB px-4 py-3" :class="{ disabled: !prodData }">
				...
				<CheckBoxGroup class="ml-5" v-if="appSettings.orgProdListLawPossibilityToShowAllProducts" v-model="showAll" @input="departmentExistsInSelectedYear" :text="getTranslation('I00.00033240', 'IncludeNotClassifiedProducts')" />
			</div>
		</template>
		...
		departmentExistsInSelectedYear() {
			...
			return axios
				...
				.then(response => {
					...
					} else window.Ticker.AddInfoMessage(this.getTranslation('I00.00027400', 'No data information'));
				})
				.catch(errorDebug);
		},
