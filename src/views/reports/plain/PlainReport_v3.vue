	...
	<div>
		<Reports @tabChanged="tabChanged" :prodData="prodData" :selectedColumns="selectedColumns" :perPage.sync="perPage" :hiddenColumns="hiddenColumns" @columnsChanged="columnsChanged" @hide-columns="hiddenColumns = $event">
			<template v-slot="data">
				<PlainList :data="data.item.products" :showCustomFieldName="data.showCustomFieldName" :orgId="data.item.orgId" :title="data.item.departmentNodes ? data.item.departmentNodes.tableName : ''" :selectedColumns="selectedColumns" :hiddenColumns="hiddenColumns" :excelTitle="data.excelTitle" @hide-columns="hiddenColumns = $event" @openAdvancedFilter="openAdvancedFilter($event)" :key="data.item.index" :canEdit="canEdit" :perPage="perPage" />
			</template>
		</Reports>
...
import PlainList from '../../../components/reports/plain/PlainList_v3';
...
export default {
	...
	methods: {
		getSelectedDepartmentList() {
			...
			return axios
				.cancelAll()
				.postWithCancelToken('/OrgProdListAll/GetDepartments', { pagestyle: this.pageStyle, selectedColumns: this.selectedColumns, orgid: node.KeyID, org_mdbid: node.KeyID_mdbID })
				.then(response => {
					if (response) {
