<template>
	<Reports @tabChanged="tabChanged" :prodData="prodData" :selectedColumns="selectedColumns" :perPage.sync="perPage" :hiddenColumns="hiddenColumns" @columnsChanged="columnsChanged" @hide-columns="hiddenColumns = $event">
		<template v-slot="data">
			<UfiList :data="data.item.products" :orgId="data.item.orgId" :showCustomFieldName="data.showCustomFieldName" :title="data.item.departmentNodes ? data.item.departmentNodes.tableName : ''" :selectedColumns="selectedColumns" :hiddenColumns="hiddenColumns" :excelTitle="data.excelTitle" @hide-columns="hiddenColumns = $event" :key="data.item.index" :canEdit="canEdit" :perPage="perPage" />
		</template>
	</Reports>
</template>
...
import UfiList from '../../../components/reports/ufi/UfiList_v3';
...
export default {
  ...
  methods: {
    getSelectedDepartmentList() {
      ...
      return axios
        .cancelAll()
        .postWithCancelToken('/OrgProdListUfi/GetDepartments', { pagestyle: this.pageStyle, selectedColumns: this.selectedColumns, orgid: node.KeyID, org_mdbid: node.KeyID_mdbID })
        .then(response => {
          if (response) {
