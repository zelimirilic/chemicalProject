<template>
	<Reports @tabChanged="tabChanged" :prodData="prodData" :perPage.sync="perPage">
		<template v-slot="data">
			<PrioList :data="data.item.products" :orgId="data.item.orgId" :title="data.item.departmentNodes ? data.item.departmentNodes.tableName : ''" :excelTitle="data.excelTitle" :key="data.item.index" :perPage="perPage" />
		</template>
	</Reports>
</template>
...
import PrioList from '../../../components/reports/remarks/PrioList_v3';
...
export default {
  ...
  methods: {
    getSelectedDepartmentList() {
      ...
      return axios
        .cancelAll()
        .postWithCancelToken('/OrgProdListPrio/GetDepartments', { pagestyle: this.pageStyle, orgid: node.KeyID, org_mdbid: node.KeyID_mdbID })
        .then(response => {
          if (response) {
