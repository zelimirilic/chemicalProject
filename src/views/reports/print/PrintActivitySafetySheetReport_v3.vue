<template>
	<div>
		<Reports @tabChanged="tabChanged" :prodData="prodData" :perPage.sync="perPage" :hiddenColumns="hiddenColumns" @hide-columns="hiddenColumns = $event" @is-selected="$emit('is-selected', $event)">
			<template v-slot="data">
				<PrintActivitySafetySheetList :data="data.item.products" :showCustomFieldName="data.showCustomFieldName" :orgId="data.item.orgId" :title="data.item.departmentNodes ? data.item.departmentNodes.tableName : ''" :selectedColumns="selectedColumns" :hiddenColumns="hiddenColumns" :excelTitle="data.excelTitle" @hide-columns="hiddenColumns = $event" @is-selected="getSelected($event)" @openAdvancedFilter="openAdvancedFilter($event)" :key="data.item.index" :canEdit="canEdit" :perPage="perPage" />
			</template>
		</Reports>
...
import PrintActivitySafetySheetList from '../../../components/reports/print/PrintActivitySafetySheetList_v3';
...
export default {
	name: 'PrintActivitySafetySheetReport',
	components: {
		Reports,
		PrintActivitySafetySheetList,
		ModalDialog,
		AdvancedFilter
	},
	data() {
	...
	methods: {
		getSelectedDepartmentList() {
			...
			return axios
				.cancelAll()
				.postWithCancelToken('/OrgProdListBatchPrintActivitySafetySheet/GetDepartments', { pagestyle: this.pageStyle, selectedColumns: this.selectedColumns, orgid: node.KeyID, org_mdbid: node.KeyID_mdbID })
				.then(response => {
					if (response) {
