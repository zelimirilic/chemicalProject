			...
			<DataTable :tableData="products" noHeader noFooter noSearch :perPage="10000" :isSearchResult="true" :hasOuterSearchWord="lastSearchObj.search_word">
				...
				<template #body="prod">
					<tr :key="prod.ID" :class="{ selectedRow: selectMultipleProducts ? findAddedProductId(prod.item) : findId(prod.item) }">
						<td class="iconCol">
							<span class="icons smallIcn circle icon-plus" v-tooltip="getTranslation('I00.00010750', 'Add product')" @click="addProduct(prod.item)" v-if="selectMultipleProducts ? !findAddedProductId(prod.item) : !findId(prod.item)"></span>
							<span class="icons smallIcn circle icon-delete active" v-tooltip="getTranslation('I00.00011100', 'Remove product')" @click="removeProduct(prod.item)" v-else></span>
						</td>
