<template>
	<div class="cardCont simpleAdd">
		<SmallLoader v-if="!countries" />
		<template v-else>
			<div class="card-body">
				<Alerts additionalClass="sticky-top" :class="{ 'mb-4': showAlert }" @close="showAlert = false" v-if="!showProductRequestModal && !addedProductsIds" />
				<Search :countries="countries" :country="country" :lastSearchObj="lastSearchObj" :numberOfProducts="(searchedProducts || []).length" @search="submitSearch" @countryChanged="country = $event" @maximize="$emit('maximize')" ref="search" />
			...
			<div class="card-footer d-flex">
				...
				<ButtonClose class="ml-auto" @click="close()" />
			</div>
			<ModalDialog :title="getTranslation('I00.00044630', 'Product request')" @close="showProductRequestModal = false" :withFooter="true" additional-class="w-40" v-if="showProductRequestModal">
...
import ProductsSimple from './add/ProductsSimple';
import ProductRequestForm from './add/ProductRequestForm';
...
export default {
	...
	methods: {
		...
		getProductRequestData() {
			axios
				.post('/ProductRequest/Show', { countryCode: this.country.IsoCountryCode, returnJson: true })
				.then((response) => {
		...
		productInfoMsg(msgType) {
			this.showAlert = true;
			alert.clearMessages();
			if (msgType) alert.addSuccessMessage(this.getTranslation('I00.00056860', 'Product successfully added.'));
			else alert.addSuccessMessage(this.getTranslation('I00.00056870', 'Product successfully removed.'));
		},
	},
	computed: {
		tabs() {
