<template>
	<DataTable :tableData="cloneData || currentData" :orgId="orgId" :perPage="perPage" :title="title" :selectFilterProps="['pictograms', 'sdsdocLanguage', 'usageAmount', 'trafficLightStatusName']" @filteredTableData="setFilteredData" @filterChanged="$emit('filter-status', $event)" :selectFilterValues="filteredDataValues" :excelTitle="excelTitle" :customFilters="customFilters" @update:per-page="$emit('update:per-page', $event)" :hiddenColumns="appSettings.useReportsAdditionalColumns && hiddenColumns" @hide-columns="$emit('hide-columns', $event)" :edit="cloneData != null" @[eventEdit]="showEdit" @cancel="cloneData = null" @save="save" showHeadersWhenEmpty>
		<template #head>
			<tr>
				...
				<Th class="prodNo" prop="flamepoint" name="flamepoint" sort export :placeholder="getTranslation('I00.00033020', 'Flame point class')">{{ getTranslation('I00.00033020', 'Flame point class') }}(°C)</Th>
				<Th prop="form" name="form" sort export :placeholder="getTranslation('I00.00016920', 'Form')">{{ getTranslation('I00.00016920', 'Form') }}</Th>
...
export default {
  ...
  props: ['data', 'subSearches', 'title', 'excelTitle', 'selectedColumns', 'hiddenColumns', 'canEdit', 'perPage', 'customFilters', 'orgId'],
  data() {
    return {
