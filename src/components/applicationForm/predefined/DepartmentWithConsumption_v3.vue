		...
		<div class="col-12">
			<div class="multiSelTree hideElm mxhp-225 mt-3 scrollBar" :class="{ oneElm: listOfSelectedItems.length <= 1 }">
				<div class="statements d-flex align-items-center" v-for="(item, index) in listOfSelectedItems" :key="index">
					<div class="col">
						<span :title="nodeNamePath(sideTree.getAllData().find((y) => y.item.orgID.id === item.orgID.id))">{{ item.name }}</span>
					</div>
...
import ConsumptionMultiple from '../../common/form/consumption/ConsumptionMultiple_v3';
